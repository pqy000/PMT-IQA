        ...
        for i in range(0, len(index)):
            ...
            for j, item in enumerate(train_sel):
                for aug in range(patch_num):
                    sample.append((imgpath[item], labels[0][item], round(labels[item] / 0.5)))
...
class BIDFolder(data.Dataset):
    ...
    def __init__(self, root, index, transform, patch_num):
        ...
        for row in rows:
            ...
            mos = np.asarray(mos)
            mos = np.asarray(mos).astype(np.float32)
            mos_all.append(mos)
            if count == 590:
                break
        ...
        for i, item in enumerate(index):
            for aug in range(patch_num):
                sample.append(
                    (os.path.join(root, 'ImageDatabase', imgname[item]), mos_all[item], round(mos_all[item] / 0.5)))
